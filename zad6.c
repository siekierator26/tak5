...
void findBigger(int numbers[], int N, float avg, float odchylenie)
{
    for (int i=0; i<N; i++) {
        if (numbers[i]>avg+odchylenie){
            printf("%d, ", numbers[i]);
        }
    }
}

void findSmaller(int numbers[], int N, float avg, float odchylenie)
{
    for (int i=0; i<N; i++) {
        if (numbers[i]<avg-odchylenie){
            printf("%d, ", numbers[i]);
...
int main(void)
{
    ...
    printf("\nWartosci mniejsze od sredniej - odchylenie: ");
    findSmaller(numbers, N, avg, odchylenie);

    printf("\nWartosci wieksze od sredniej + odchylenie: ");
    findBigger(numbers, N, avg, odchylenie);
    return 0;
